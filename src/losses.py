...
def rpn_loss_regr(num_anchors):
  def rpn_loss_regr_fixed_num(y_true, y_pred):
    ...
    x_abs = tf.abs(x)
    x_bool = tf.less_equal(x_abs, 1.0).as_type(tf.float32) # |x| < 1
...
def class_loss_regr(num_classes):
  def class_loss_regr_fixed_num(y_true, y_pred):
    ...
    x_abs = tf.abs(x)
    x_bool = tf.less_equal(x_abs, 1.0).as_type(tf.float32) # |x| < 1
    return lambda_rpn_regr * tf.reduce_sum(y_true[:, :, :4*num_classes] * (x_bool * 0.5 * x * x + (1-x_bool) * (x_abs-0.5))) / tf.reduce_sum(epsilon+y_true[:, :, :4*num_classes])
  return class_loss_regr_fixed_num

def class_loss_cls(y_true, y_pred):
  return lambda_cls_class * tf.reduce_mean(keras.losses.categorical_crossentropy(y_true[0, :, :], y_pred[0, :, :]))